...
public class GraphDemoFrame extends JFrame {
    ...
    private JComboBox<String> comboBoxGraphList;
    private List<Graph> generatedGraphs = new ArrayList<>();
    // -----------------------------------
    ...
    public GraphDemoFrame() {
        ...
        initGraphvizPainter();
        initExamplesCombo();
        initGraphTabListeners();
        initGraphvizTabListeners();

        // ---------- СОЗДАНИЕ ВКЛАДКИ "ГЕНЕРАЦИЯ" ----------
        JPanel panelGenerateTab = new JPanel(new BorderLayout(10, 10));
        panelGenerateTab.setBorder(BorderFactory.createEmptyBorder(10, 10, 10, 10));

        // Левая панель с управлением – используем BorderLayout, чтобы элементы были сверху
        JPanel leftPanel = new JPanel(new BorderLayout());
        leftPanel.setBorder(BorderFactory.createTitledBorder("Параметры"));

        // Панель с элементами управления (вертикальный Box, но без растяжения)
        ...
        spinnerN = new JSpinner(new SpinnerNumberModel(3, 2, 6, 1));
        nPanel.add(spinnerN);
        nPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
        controlsPanel.add(nPanel);

        // Строка 2: кнопка генерации
        JPanel genPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        buttonGenerate = new JButton("Сгенерировать");
        genPanel.add(buttonGenerate);
        genPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
        controlsPanel.add(genPanel);

        // Строка 3: выпадающий список
        JPanel listPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        listPanel.add(new JLabel("Выберите граф:"));
        comboBoxGraphList = new JComboBox<>();
        comboBoxGraphList.setPreferredSize(new Dimension(200, 25));
        listPanel.add(comboBoxGraphList);
        listPanel.setAlignmentX(Component.LEFT_ALIGNMENT);
        controlsPanel.add(listPanel);

        // Строка 4: информация о количестве
        JPanel infoPanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        JLabel infoLabel = new JLabel("Количество графов: ");
        ...
        // Добавляем распорку, чтобы элементы прижимались к верху (но это не обязательно)
        controlsPanel.add(Box.createVerticalGlue());

        // Помещаем controlsPanel в верхнюю часть leftPanel
        leftPanel.add(controlsPanel, BorderLayout.NORTH);

        // Правая панель для отображения графа
        JPanel rightPanel = new JPanel(new BorderLayout());
        SvgPanel genPainter = new SvgPanel();
        rightPanel.add(new JScrollPane(genPainter), BorderLayout.CENTER);

        JSplitPane splitGen = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, leftPanel, rightPanel);
        splitGen.setResizeWeight(0.3);
        panelGenerateTab.add(splitGen, BorderLayout.CENTER);

        tabbedPaneMain.addTab("Генерация", panelGenerateTab);

        // Обработчики для генерации
        buttonGenerate.addActionListener(e -> {
            int n = (int) spinnerN.getValue();
            System.out.println("Нажата кнопка генерации для N=" + n);
            countLabel.setText("Генерация...");
            comboBoxGraphList.removeAllItems();
            comboBoxGraphList.setEnabled(false);

            SwingWorker<List<Graph>, Void> worker = new SwingWorker<List<Graph>, Void>() {
                @Override
                protected List<Graph> doInBackground() throws Exception {
                    return GraphGenerator.generate(n);
                }
                ...
                protected void done() {
                    try {
                        generatedGraphs = get();
                        long total = GraphGenerator.totalCount;
                        if (total == 0) total = generatedGraphs.size();

                        countLabel.setText(String.valueOf(total));
                        comboBoxGraphList.removeAllItems();

                        if (generatedGraphs.isEmpty()) {
                            comboBoxGraphList.addItem("Графов не найдено");
                        } else {
                            for (int i = 0; i < generatedGraphs.size(); i++) {
                                comboBoxGraphList.addItem("Граф #" + (i + 1));
                            }
                            if (total > generatedGraphs.size()) {
                                comboBoxGraphList.addItem("... ещё " + (total - generatedGraphs.size()) + " графов");
                            }
                            comboBoxGraphList.setSelectedIndex(0);
                            showGeneratedGraph(genPainter, 0);
                        }
                        comboBoxGraphList.setEnabled(true);

                        // Вывод в текстовое поле (без использования showSystemOut)
                        /*SwingUtilities.invokeLater(() -> {
                            textAreaSystemOut.setText("N = " + n + ", число графов: " + total);
                        });*/
                    } catch (Exception ex) {
        ...
        comboBoxGraphList.addActionListener(e -> {
            int idx = comboBoxGraphList.getSelectedIndex();
            if (idx >= 0 && idx < generatedGraphs.size()) {
                showGeneratedGraph(genPainter, idx);
            }
        });
        // -------------------------------------------------
    }

    // Вспомогательный метод для отображения сгенерированного графа
    private void showGeneratedGraph(SvgPanel painter, int index) {
        Graph g = generatedGraphs.get(index);
        try {
            String dot = GraphUtils.toDot(g);
            String svg = dotToSvg(dot);
            painter.paint(svg);
        } catch (Exception ex) {
            SwingUtils.showErrorMessageBox(ex);
        }
    ...
    private void initGraphvizPainter() {
    ...
    private void initExamplesCombo() {
    ...
    private void initGraphTabListeners() {
        ...
        buttonSaveGraphSvgToFile.addActionListener(e -> {
            if (panelGraphPainter.svg == null) return;
    ...
    private void initGraphvizTabListeners() {
        ...
        buttonSaveDotSvgToFile.addActionListener(e -> {
            if (panelGraphvizPainter.svg == null) return;
            if (fileChooserImgSave.showSaveDialog(GraphDemoFrame.this) == JFileChooser.APPROVE_OPTION) {
                String filename = fileChooserImgSave.getSelectedFile().getPath();
                if (!filename.toLowerCase().endsWith(".svg")) {
                    filename += ".svg";
    ...
    // ---------- Сгенерированный код GUI (не меняем) ----------
